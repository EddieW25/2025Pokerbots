...
class PokerEnv:
    ...
    def deal_community_cards(self):
        if self.betting_round == 1:  # Flop
            ...
            self.current_board.extend(flop)
        elif self.betting_round in [2, 3]:  # Turn, River
            self.community_cards.append(self.deck.pop())
            self.current_board.append(self.deck.pop())
    ...
    def calculate_final_reward(self):
        ...
        if player_score > opponent_score:
            return 1.5 * self.pot + 10 if self.bounty in self.current_board else self.pot
...
if __name__ == "__main__":
    bounty = random.randint(0,12)
    NUM_CARDS = 52  # Assuming this constant is defined
    env = PokerEnv(bounty)  # Assuming the environment is initialized correctly
